...
class RuleProcessor:
    ...
    def __init__(self, rules_folder=None):
        ...
        Args:
            rules_folder (str): Path to folder containing rule JSON files
        """
        self.rules_folder = rules_folder or os.path.join(PROJECT_ROOT, 'backend', 'rules')
        self.db = DataAccess()
    ...
    def _apply_table_lookup(self, rule_json, input_values):
        """
        Apply table lookup logic for toxicity grading with enhanced scenarios handling.

        Scenarios:
        1. No relevant measurements found → "Unknown"
        2. Relevant measurements found but none match any grade → "No fit"
        3. Relevant measurements match a grade → Return grade
        """
        ...
        # Track if we have any relevant measurements
        has_any_relevant_measurement = False

        # Check what measurements we actually have
        for param_name in lookup_table.get("GRADE I", {}).keys():  # Get parameter names from any grade
        ...
        # Scenario 2 & 3: We have relevant measurements, check if any match grades
        ...
        # Scenario 3: Found a matching grade
        if max_grade_name:
            return max_grade_name

        # Scenario 2: Have relevant measurements but no grade matches
        return "No fit"

    def _apply_combination_lookup(self, rule_json, input_values):
        """
        Enhanced combination lookup with improved treatment error handling.
        """
        rules = rule_json.get('rules', {})
        input_params = rule_json.get('input_parameters', [])
        rule_name = rule_json.get('rule_name', '')

        # Check if this is a treatment rule
        is_treatment_rule = 'treatment' in rule_name.lower()

        # Create the combination key and track missing/problematic parameters
        param_values = []
        missing_params = []
        no_fit_params = []
        ...
        if is_treatment_rule:
            if missing_params:
                return f"Missing information for sub state: {', '.join(missing_params)}"
            elif no_fit_params:
                return "No exact match to the protocol"
            else:
                return "No exact match to the protocol"
    ...
    def process_patient_rules(self, patient_id, df):
        ...
        results = {
            ...
            "treatment_recommendations": "No fit"  # Default, may be overwritten
        }
