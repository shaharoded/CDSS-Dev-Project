...
class CDSSApp(tk.Tk):
    ...
    def _setup_ui(self):
        ...
        labels = ['First Name', 'Last Name', 'LOINC Code', 'Concept Name', 'Value', 'Unit', 'Valid Start Time', 'Transaction Time']
        placeholders = {
            'First Name': 'e.g., John',
            'Last Name': 'e.g., Doe',
            'LOINC Code': 'e.g., 1234-5',
            'Concept Name': 'e.g. Temperature',
            'Value': 'e.g., 5.6',
            'Unit': 'e.g., mg/dL',
        ...
        tooltips = {
            ...
            'Concept Name':  'Required for Insert / Update / Delete operations',
    ...
    def delete_measurement(self):
        try:
            result = bs.PatientRecord.delete_measurement(
                first_name=self._get('First Name'),
                last_name=self._get('Last Name'),
                loinc_num=self._get('LOINC Code'),
                component_name=self._get('Concept Name'),
                valid_start_time=self._get('Valid Start Time')
            )


            # Clear listbox and show deleted rows
            self.listbox.delete(0, tk.END)
            if result:
                header = f"{'LOINC':<12} {'Concept Name':<20} {'Value':<10} {'Unit':<10} {'Valid Start':<18} {'Transaction Time':<18}"
                self.listbox.insert(tk.END, "Deleted Measurements:")
                self.listbox.insert(tk.END, header)
                self.listbox.insert(tk.END, "-" * len(header))
                for row in result:
                    loinc, concept, val, unit, vstart, ttime = row[:6]
                ...
                self.listbox.insert(tk.END, "No rows were deleted.")
